...
let SseStream = require('ssestream2');
...
exports.updateIssue = function (req, res, next) {
  ...
  console.log('addFeedbackExtra2()');
    ...
    let imgFile0 = validator.trim(req.body.imgFile0);
    let imgFile1 = validator.trim(req.body.imgFile1);
    let imgFile2 = validator.trim(req.body.imgFile2);

    let uploadDir = path.join(__dirname + "/../public/upload");

    helpModel.addFeedbackExtra2(chip,model,mac,activeid,ccosver,category,title,content,contact,imgFile0,imgFile1,imgFile2,uploadDir,function(err, result) {
      if(err) {
          return output.error(req,res,err);
      } else {
          console.log(JSON.stringify(result));
